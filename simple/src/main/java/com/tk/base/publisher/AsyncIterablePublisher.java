...
/**
 * @Description: Publisher的实现，使用指定的Executor异步执行，并为指定的Iterable生成元素。
 * 以unicast的形式为Subscriber指定的Iterable生成元素。
 * <p>
 * 代码中使用了很多try-catch，用于展示什么时候可以抛异常，什么时候不能抛异常。
 * @Date : 2023/04/04 19:23
 * @Auther : tiankun
 */
public class AsyncIterablePublisher<T> implements Publisher<T> {
     ...
     * @param executor  线程池
     */
    public AsyncIterablePublisher(final Iterable<T> elements, final int
    ...
    final class SubscriptionImpl implements Subscription, Runnable {
            ...
            try {
                ...
                do {
                    ...
                    try {
                        ...
                        // If `next` or `hasNext` throws (they can, sinceit is user-provided), we need to treat the stream as errored as per rule1.4
                    ...
                    if (!hasNext) {
                        ...
                        // 首先考虑是票据取消了订阅
                        doCancel();
                        // Then we signal `onComplete` as per rule 1.2 and1.5
                        // 发送onComplete信号给订阅者
                        subscriber.onComplete();
                    }
                ...
                // If the `Subscription` is still alive and well, and wehave demand to satisfy, we signal ourselves to send more data
                // 如果订阅票据没有取消，还有请求，通知自己发送更多的数据

                if (!cancelled && demand > 0)
                    signal(Send.Instance);
            } catch (final Throwable t) {
                // We can only get here if `onNext` or `onComplete` threw,and they are not allowed to according to 2.13, so we can only cancel andlog here.
        ...
        private void terminateDueTo(final Throwable t) {
            ...
            try {
                ...
                // 如果onError抛异常，只能记录信息。
                (new IllegalStateException(subscriber + " violated the Reactive Streams rule 2.13 by throwing an exception from onError.",
